...
public class UserInterceptor implements Interceptor {
    public void intercept(Invocation ai) {
        ...
        if (user == null) {
            ...
            try {
                if (uri.contains("parents"))
                    ai.getController().getResponse().getWriter().write("<script>alert('请先登录');location.href=\'/parents/login\'</script>");
                else
                    ai.getController().getResponse().getWriter().write("<script>location.href=\'/school/login\'</script>");
            } catch (IOException e) {
                e.printStackTrace();
            }
        } else {
